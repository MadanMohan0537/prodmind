...
function render(result) {
  ...
  $("#topics").innerHTML = result.topics.map(topic => `<article class="topic"><header><h3>${escapeHtml(topic.label)}</h3><span>${topic.documentCount} docs · ${Math.round(topic.share * 100)}%</span></header><div class="bar"><i style="width:${topic.share * 100}%"></i></div><div class="chips">${topic.keywords.map(keyword => `<span class="chip">${escapeHtml(keyword.term)}</span>`).join("")}</div></article>`).join("");
  $("#hierarchy").innerHTML = `<h3>Topic hierarchy</h3>${result.hierarchy.map(group => `<div class="group"><b>${escapeHtml(group.label)}</b> → ${group.children.map(escapeHtml).join(", ")}</div>`).join("")}`;
}
