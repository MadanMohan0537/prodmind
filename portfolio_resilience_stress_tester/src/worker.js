import {stressPortfolio} from './stress.js';
...
export default {
  async fetch(request, env) {
    ...
    if (url.pathname === '/api/health') return json({service:'prodmind-portfolio-resilience', configured:Boolean(env.API_TOKEN)});
    ...
    if (url.pathname !== '/api/stress' || request.method !== 'POST') return json({error:'Not found'}, 404);
    try {
      const input = await readBody(request);
      return json(stressPortfolio(input.runs, input.strategy, input.options));
    } catch (error) {
