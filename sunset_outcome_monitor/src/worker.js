import {reviewSunsetOutcomes} from './outcomes.js';

const security = {'Cache-Control':'no-store','X-Content-Type-Options':'nosniff','Referrer-Policy':'no-referrer','Content-Security-Policy':"default-src 'self'; script-src 'self'; style-src 'self'; connect-src 'self'; frame-ancestors 'none'; base-uri 'none'; form-action 'self'"};
...
async function authorized(request, token) {
  ...
  const [left,right] = await Promise.all([digest(request.headers.get('Authorization')??''),digest(`Bearer ${token}`)]);
...
export default {async fetch(request,env){
  ...
  if(url.pathname==='/api/health') return json({service:'prodmind-sunset-outcome-monitor',configured:Boolean(env.API_TOKEN)});
  ...
  if(url.pathname!=='/api/sunset-outcomes'||request.method!=='POST') return json({error:'Not found'},404);
  try {const body=await readBody(request);return json(reviewSunsetOutcomes(body.runs,body.sunsetReport,body.input));}
  catch(error){return json({error:error.message},error instanceof RangeError?413:422)}
}};
