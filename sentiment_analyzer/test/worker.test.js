...
test("corrections fail clearly without D1",async()=>{const r=await route(new Request("https://x/api/corrections",{method:"POST",headers:{Authorization:"Bearer secret","Content-Type":"application/json"},body:JSON.stringify({analysisId:"a",correctedLabel:"positive"})}),env);assert.equal(r.status,503)});
test("metrics fail clearly without D1",async()=>{const r=await route(new Request("https://x/api/metrics",{headers:{Authorization:"Bearer secret"}}),env);assert.equal(r.status,503)});
