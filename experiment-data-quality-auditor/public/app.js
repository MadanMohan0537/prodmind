...
$('#portfolio-stress').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a stress scenario JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/portfolio-stress',input);const parent=$('#stress-results');parent.replaceChildren();
  const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({'Portfolio status':result.status,'Mean resilience':result.summary.meanResilienceScore,'Weakest scenario':result.summary.weakestScenarioId,'Critical scenarios':result.summary.critical})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));
  for(const scenario of result.scenarios){const card=el('article',undefined,'card');card.append(el('h3',scenario.name),el('p',`${scenario.status} · score ${scenario.resilienceScore} · capacity overrun ${scenario.capacityOverrun}`,'muted'));details(card,'Inspect assumptions, allocation, and at-risk evidence',scenario);parent.append(card);}notice('Portfolio stress test complete. No saved ranking was changed.');
});};
$('#benefits-realization').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a benefits JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/benefits-realization',input);const parent=$('#benefits-results');parent.replaceChildren();
  const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({'Benefits':result.summary.benefits,'Measured':result.summary.measured,'Realized':result.summary.realized,'Overdue':result.summary.overdue})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));
  for(const item of result.benefits){const card=el('article',undefined,'card');card.append(el('h3',item.name),el('p',`${item.status} · ${item.progress===null?'not measured':`${Math.round(item.progress*100)}% of target`} · owner ${item.owner}`,'muted'));details(card,'Inspect targets, attribution and evidence lineage',item);parent.append(card);}notice('Benefits ledger calculated. Review attribution before drawing conclusions.');
});};
$('#investment-assurance').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an assurance JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/investment-assurance',input);const parent=$('#assurance-results');parent.replaceChildren();
  const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({'Reviews':result.summary.reviews,'Complete':result.summary.complete,'Incomplete':result.summary.incomplete,'Overdue actions':result.summary.overdueActions})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));
  for(const review of result.reviews){const card=el('article',undefined,'card');card.append(el('h3',review.title),el('p',`${review.assuranceStatus} · decision ${review.decision} · reviewer ${review.reviewer}`,'muted'));details(card,'Inspect checks, benefits, lineage, and actions',review);parent.append(card);}notice('Investment assurance review complete. The decision remains human judgment.');
});};
$('#assumption-risk').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an assumptions JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/assumption-risk',input);const parent=$('#assumption-results');parent.replaceChildren();
  const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({'Assumptions':result.summary.assumptions,'Untested':result.summary.untested,'Overdue':result.summary.overdue,'Unlinked':result.summary.unlinked})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));
  for(const item of result.assumptions){const card=el('article',undefined,'card');card.append(el('h3',item.statement),el('p',`${item.category} · ${item.status} · exposure ${item.exposureScore} · owner ${item.owner}`,'muted'));details(card,'Inspect validation plan and authoritative lineage',item);parent.append(card);}notice('Assumption register assessed. Review the validation queue before committing more capacity.');
});};
$('#release-readiness').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a release JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/release-readiness',input);const parent=$('#release-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Releases:result.summary.releases,Ready:result.summary.ready,Blocked:result.summary.blocked,'Failed checks':result.summary.failedChecks})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const release of result.releases){const card=el('article',undefined,'card');card.append(el('h3',`${release.title} · ${release.version}`),el('p',`${release.status} · ${release.strategy} · ${release.failedChecks.length} failed checks`,'muted'));details(card,'Inspect rollout, monitors, rollback and lineage',release);parent.append(card);}notice('Release readiness assessed. No deployment action was taken.');
});};
$('#feature-adoption').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an adoption JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/feature-adoption',input);const parent=$('#adoption-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Journeys:result.summary.journeys,Events:result.summary.events,Users:result.summary.users,'Invalid users':result.summary.invalidUsers})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const journey of result.journeys){const card=el('article',undefined,'card');card.append(el('h3',`${journey.title} · ${journey.releaseVersion}`),el('p',`${Math.round(journey.completionRate*100)}% completion · ${journey.medianTimeToValueMinutes??'—'} median minutes to value · ${journey.readinessStatus}`,'muted'));details(card,'Inspect funnel, segments and release lineage',journey);parent.append(card);}notice('Feature adoption analyzed. No causal claim or product decision was made.');
});};
$('#product-lifecycle').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a lifecycle JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/product-lifecycle',input);const parent=$('#lifecycle-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Plans:result.summary.plans,Ready:result.summary.ready,Blocked:result.summary.blocked,'Observed users':result.summary.affectedUsers})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const plan of result.plans){const card=el('article',undefined,'card');card.append(el('h3',`${plan.title} · ${plan.action}`),el('p',`${plan.status} · ${plan.noticeDays??'—'} notice days · ${plan.failedChecks.length} failed checks`,'muted'));details(card,'Inspect adoption, migration, communications, approvals and lineage',plan);parent.append(card);}notice('Lifecycle plan validated. No feature or customer workflow was changed.');
});};
$('#sunset-migration').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a migration JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/sunset-migration',input);const parent=$('#sunset-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Snapshots:result.summary.snapshots,Ready:result.summary.ready,Hold:result.summary.hold,'Overdue hold':result.summary.overdueHold,Remaining:result.summary.remaining})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const item of result.snapshots){const card=el('article',undefined,'card');card.append(el('h3',`${item.title} · ${item.status}`),el('p',`${Math.round(item.migration.coverage*100)}% reconciled · ${item.migration.remaining} remaining · ${item.failedChecks.length} failed checks`,'muted'));details(card,'Inspect cohorts, dependencies, notices, exceptions, telemetry and approvals',item);parent.append(card);}notice('Sunset readiness assessed. No migration or shutdown action was taken.');
});};
$('#sunset-outcomes').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an outcome JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/sunset-outcomes',input);const parent=$('#sunset-outcome-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Reviews:result.summary.reviews,Closed:result.summary.closed,'Action required':result.summary.actionRequired,'Blocked close':result.summary.blockedClose,'Overdue actions':result.summary.overdueActions})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const item of result.reviews){const card=el('article',undefined,'card');card.append(el('h3',`${item.title} · ${item.status}`),el('p',`${item.failedChecks.length} failed checks · ${Math.round(item.economics.realizationRate*100)}% savings realization · ${item.residualTrafficCount} residual requests`,'muted'));details(card,'Inspect outcomes, reversibility, actions and human decision',item);parent.append(card);}notice('Sunset outcome reviewed. No close or restore action was taken.');
});};
$('#governance-pack').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a governance JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/governance-pack',input);const parent=$('#governance-pack-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Packs:result.summary.packs,Certified:result.summary.certified,'Action required':result.summary.actionRequired,'Blocked certification':result.summary.blockedCertification})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const item of result.packs){const card=el('article',undefined,'card');card.append(el('h3',`${item.title} · ${item.status}`),el('p',`${item.artifacts.length} artifacts · ${item.failedChecks.length} failed checks · ${item.classification}`,'muted'),el('p',`Pack SHA-256: ${item.packDigest}`,'mono'));details(card,'Inspect artifact chain, governance checks, approvals and certification',item);parent.append(card);}notice('Governance pack built. No legal certification or digital signature was created.');
});};
$('#governance-obligations').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose a governance monitor JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/governance-obligations',input);const parent=$('#governance-obligation-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Monitors:result.summary.monitors,Current:result.summary.current,'Action required':result.summary.actionRequired,'Blocked continue':result.summary.blockedContinue,Overdue:result.summary.overdueObligations,'Digest mismatches':result.summary.digestMismatches})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const item of result.monitors){const card=el('article',undefined,'card');card.append(el('h3',`${item.title} · ${item.status}`),el('p',`${item.obligations.length} obligations · ${item.failedChecks.length} failed checks · next review ${item.nextReviewAt}`,'muted'));details(card,'Inspect obligations, time windows, digest and human review',item);parent.append(card);}notice('Governance obligations reviewed. No recertification or disposition was performed.');
});};
$('#governance-exceptions').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an exception register JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/governance-exceptions',input);const parent=$('#governance-exception-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Registers:result.summary.registers,Exceptions:result.summary.exceptions,Active:result.summary.active,'Blocked approvals':result.summary.blockedApprovals,Rejected:result.summary.rejected,Closed:result.summary.closed,Expired:result.summary.expired})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const register of result.registers){for(const item of register.exceptions){const card=el('article',undefined,'card');card.append(el('h3',`${register.title} · ${item.status}`),el('p',`${item.targetType} ${item.targetId} · ${item.riskRating} risk · upstream ${register.underlyingStatus}`,'muted'));details(card,'Inspect scope, expiry, controls, remediation, approvals and decision',item);parent.append(card);}}notice('Exceptions reviewed. No failed upstream result or product control was changed.');
});};
$('#exception-exits').onsubmit = event => {event.preventDefault();action(async()=>{
  const file=event.currentTarget.elements.file.files[0];if(!file||file.size>2_000_000)throw new Error('Choose an exception exit JSON file under 2 MB.');const input=JSON.parse(await file.text());const result=await api('/api/exception-exits',input);const parent=$('#exception-exit-results');parent.replaceChildren();const metrics=el('div',undefined,'metrics');for(const[label,value]of Object.entries({Reviews:result.summary.reviews,'Verified closed':result.summary.verifiedClosed,'Blocked close':result.summary.blockedClose,'Action required':result.summary.actionRequired,'Failed tests':result.summary.failedEffectivenessTests})){const tile=el('div',undefined,'metric');tile.append(el('strong',String(value)),el('span',label));metrics.append(tile);}parent.append(metrics,el('p',result.method,'muted'));for(const item of result.reviews){const card=el('article',undefined,'card');card.append(el('h3',`${item.title} · ${item.status}`),el('p',`${item.observedDays}/${item.observationWindowDays} observed days · ${item.residualRisk} residual risk`,'muted'));details(card,'Inspect target, remediation, tests, observation, approvals and decision',item);parent.append(card);}notice('Exception exits verified. No exception or product control was changed.');
});};
